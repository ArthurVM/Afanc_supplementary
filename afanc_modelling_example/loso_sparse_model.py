...
def parse_ardal_allele(allele_id):
    ...
    ## prefer Ardal decoding for dotted ids
    ## keeps contigs and positions tied to the headers
...
def estimate_lineage_frequencies(sample_alleles, labels_by_sample, selected_alleles, smoothing_alpha=1.0):
    ...
    for allele in selected_alleles:
        ...
        ## then collapse non targets into shared background
        for lineage in lineages:
...
def score_samples_sparse_tables(sample_alleles, selected_alleles, lineages, priors, lineage_freqs, pooled_background):
    ...
    for sample_id, aset in sample_alleles.items():
        ...
        for lineage in lineages:
            logp = math.log(_safe_prob(priors[lineage]))
            ## sparse scoring uses observed and absent markers
            ## lins with lots of markers accumulate evidence
            for allele in selected_alleles:
                target_lineage = max(lineage_freqs[allele], key=lineage_freqs[allele].get)
...
def _validate_parent_map(parent_map, leaf_labels):
    ...
    def visit(node):
        ## DFS cycle check is the simple way through this
        ## easier than untangling malformed nesting later
        if node in visited:
            return
        if node in visiting:
...
def _build_node_training_meta(meta, lineage_col, node, parent_map, children_by_parent, leaf_set):
    if node == HIERARCHY_ROOT_NODE:
        ## synthetic root only exists for disconnected roots
        ## relabel leaves to their top branch here
    ...
    ## each local node competes only among direct children
    ## but trains on all leaves below this node
...
def _build_candidate_pool(
    ...
    ## interleave lineage rankings before LOSO
    ## stops one lineage from taking the whole candidate pool
    ordered_candidates = []
    seen = set()
...
def _supplement_lineage_markers(
    ...
    ## weak lineages get topped up but rich ones keep their markers
...
def _build_flat_sparse_node_model(
    ...
    ## LOSO folds only work when held out labels exist in training
    ## otherwise the fold cannot score all classes
    folds = []
    ...
    for fold in folds:
        ...
        ## evaluate prefix panels in order
        ## LOSO should choose the smallest strong local model
        for allele in ordered_candidates:
    ...
    ## stable alleles are fold winners seen often enough
    ## then top up if a lineage is still thin
...
def build_geolineage_min_model_sparse(
    ...
    if parent_map is None:
        ## keep flat behaviour unless hierarchy is explicit
    ...
    ## each node only competes among direct children
    nodes_to_train = []
    if len(roots) > 1:
    ...
    top_classifier_node = None
    if len(roots) > 1:
        ## multiple roots need a synthetic top classifier
        ## inference needs an explicit first decision
